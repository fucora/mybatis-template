...
package com.vonchange.mybatis.tpl.named;

import com.vonchange.mybatis.tpl.exception.InvalidDataAccessApiUsageException;
import com.vonchange.mybatis.tpl.extra.DynamicSql;
import com.vonchange.mybatis.common.util.Assert;
...
public abstract class NamedParameterUtils {
		...
		for (int i = 0; i < paramNames.size(); i++) {
			...
			if (paramSource != null && paramSource.hasValue(paramName)) {
				...
				if (value instanceof Object[]||value instanceof Collection) {
					actualSql.append(list(paramName,""));
				}
				else {
					actualSql.append(paramNameMy);
				}
			}
			else {
				actualSql.append(paramNameMy);
	...
	public static  String list(String named,String itemProperty){
		String str= "<foreach collection=\"{0}\" index=\"index\" item=\"item\"  separator=\",\">  \n" +
				"            #'{'item{1}'}'" +
				"    </foreach>";
		return DynamicSql.format(str,named,itemProperty);
	}
					...
					if (value instanceof Collection) {
						...
						while (entryIter.hasNext()) {
							...
							if (entryItem instanceof Object[]) {
								...
								paramList.addAll(Arrays.asList(expressionList));
							}else{
